  ...
  if (maintenanceMode) {
    ...
    if (mfrc522.PICC_IsNewCardPresent()) {
      ...
      if (mfrc522.PICC_ReadCardSerial()) {
        int songId = getSongFromUID(mfrc522.uid.uidByte, mfrc522.uid.size);
        Serial.printf("Tag corresponds to Song ID: %d\n", songId);
...
int getSongFromUID(byte *uid, byte size) {
  ...
  const byte tag1[] = {0x04, 0x6F, 0x7F, 0xF2, 0x2F, 0x4D, 0x81}; // Pil
  const byte tag2[] = {0x04, 0x1B, 0x7E, 0xF2, 0x2F, 0x4D, 0x81}; // Bjørne brødre
  const byte tag3[] = {0x04, 0x23, 0x7E, 0xF2, 0x2F, 0x4D, 0x81}; // Amyl and the Sniffers
  const byte tag4[] = {0x04, 0x77, 0x7F, 0xF2, 0x2F, 0x4D, 0x81}; // Nik & Jay
  const byte tag5[] = {0x04, 0x7F, 0x7F, 0xF2, 0x2F, 0x4D, 0x81};
  const byte tag6[] = {0x04, 0x86, 0x7E, 0xF2, 0x2F, 0x4D, 0x81};
  const byte tag7[] = {0x04, 0x8E, 0x7E, 0xF2, 0x2F, 0x4D, 0x81}; // Medina
